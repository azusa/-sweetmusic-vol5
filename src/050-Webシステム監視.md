...
mackerel-agent v0.54.0より、`/proc/meminfo`より`MemAvailable`が取得な
...
なお、RHEL6.6/CentOS6.6の環境では、`MemAvailable`を取得するには、`/etc/sysctl.conf`に`vm.meminfo_legacy_layout = 0`を設定し[@lst:code_050_code001]、`sysctl -p`コマンドで設定を反映する必要があります。
...
command = "mackerel-plugin-postgres -user=jira -password=jira"
```
