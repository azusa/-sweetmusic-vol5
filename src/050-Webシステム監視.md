...
なお、RHEL6.6/CentOS6.6の環境では、`MemAvailable`を取得するには、[@lst:code_050_code001]のように`/etc/sysctl.conf`に`vm.meminfo_legacy_layout = 0`を設定し、`sysctl -p`コマンドで設定を反映する必要があります。
