...
各プラグインのコマンドは`/usr/bin`以下にインストールされるので、
コマンドの仕様に則ってmackerel-agent.confに設定を行います。
...
実行するコマンドを記述することができます。[@lst:code_070_code_03]は、この機能を使って、
httpのヘルスチェックに失敗した際にサービスの自動再起動を行うサンプルです。

```{#lst:code_070_code_03 caption="restart-jira.sh"}
#!/bin/bash
...
この例では、一回前のチェック監視の結果である`MACKEREL_PREVIOUS_STATUS`と直前のチェック監視の結果である`MACKEREL_STATUS`がともに`CRITICAL`の場合にサービス再起動を行いますので、
...
`command`に`--return`オプションを付与することにより、Mackerelにエラーログの内容を
出力することができます。([@fig:img_070_01_image])この際は認証情報やユーザーの個人情報などがログ内に出力されない
ように留意する必要があります。

![jpsコマンドの出力](img/check-log.png){#fig:img_070_01_image}
...
復旧法を抑止するためには、[@lst:code_070_code_04]の例のように、`mackerel-agent.conf`の設定に`prevent_alert_auto_close = true`を設定します。
...
[@lst:code_070_code_05]と[@lst:code_070_code_06a]および[@lst:code_070_code_06b] は、[@lst:code_070_code_02]で行っていたサーバーの
ヘルスチェックの監視を、プラグインを使って行うものです。


- 1分ごとにヘルスチェックを実行し、ヘルスチェックが失敗した場合は`WARING`を出力
...
```{#lst:code_070_code_06a caption="独自プラグインによる監視"}
#!/bin/bash
set -ux
...
```{#lst:code_070_code_06b caption="独自プラグインによる監視(続き)"}
if [ $RET -eq 0 ]; then
  echo 0 > ${FILE}
  exit 0
else
...
ここまで述べてきたように`check-log`監視ではログファイルのチェックを行単位で行うようになっていますが、[@lst:code_070_code_07]のようにプログラムのスタックトレースの出力は複数行を1レコードとして出力されるため、チェック監視の条件を設定する上で複数行をまとめて読み込む必要がある場合があります。

```{#lst:code_070_code_07 caption="スタックトレースの出力"}
2018-04-01 17:34:09,981 http-nio-8080-exec-5 ERROR anonymous 
java.lang.RuntimeException: org.apache.catalina.connector.ClientAbortException:
        at com.atlassian.plugin.cache.filecache.impl.StreamsCache
```
...
[plugin.checks.jira_log]
command = "check-log --file /var/atlassian/application-data/jira/log/jira.log.json --pattern ERROR --exclude ClientAbortException --return"
prevent_alert_auto_close = true
```
