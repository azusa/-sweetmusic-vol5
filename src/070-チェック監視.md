...
しかし、チェック監視の運用が生まれる背景には、汎用機のバッチ処理でエラーが発生したら運用者が対応するですとか、ハードウェアの診断機能が故障を検出したら保守ベンダーに対応を依頼するというような、そのようなITサービス運用の現場の作業フローをそのままWebシステムに適用した結果、チェック監視中心の運用フローが生まれたということもあります。
